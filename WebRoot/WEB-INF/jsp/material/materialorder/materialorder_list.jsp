				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<!-- 检索  -->
							<form
								action="materialorder/list.do?STATUS=${pd.STATUS}&TYPE=${pd.TYPE}"
...
<!-- 										</select></td> -->
													...
													<c:forEach items="${varList}" var="var" varStatus="vs">
														<tr>
															<td class='center'><label class="pos-rel"><input
																	...
																	value="${var.MATERIAL_ORDER_ID}"  class="ace" /><span
															...
															<td class='center'><c:forEach items="${var.pics}"
																	var="pic" varStatus="vars">
																	<a class="btn btn-xs btn-success" title="编辑"
																		onclick="edit('${pic}');"> pic${vars.index} </a>
																</c:forEach></td>
															<td class="center"><c:if
																...
																<div class="btn-group">
																	<c:if test="${pd.STATUS == 1&&QX.edit == 1 }">
																		<a class="btn btn-xs btn-success" title="编辑"
																			onclick="update('${var.MATERIAL_ORDER_ID}',3);">
																			审核 </a>
																	</c:if>
																	<c:if test="${pd.STATUS == 3&&QX.edit == 1 }">
																		<a class="btn btn-xs btn-success" title="编辑"">
																			已审批 </a>
																	</c:if>
																	<c:if test="${pd.STATUS == 1 && QX.del == 1 }">
																		<a class="btn btn-xs btn-danger"
																			onclick="update('${var.MATERIAL_ORDER_ID}',2);">
																			退回 </a>
																	</c:if>
																	<c:if test="${pd.STATUS == 2 &&QX.del == 1 }">
																		<a class="btn btn-xs btn-danger"> 已退回 </a>
																	</c:if>
																</div>
																		...
																		<ul
																			...
																			<c:if test="${QX.edit == 1 }">
																				<li><a style="cursor: pointer;"
																					onclick="edit('${var.MATERIALORDER_ID}');"
																			...
																			<c:if test="${QX.del == 1 }">
																				<li><a style="cursor: pointer;"
																					onclick="del('${var.MATERIALORDER_ID}');"
	...
	<script type="text/javascript">
		...
		function update(ID,type){
			var url = "<%=basePath%>materialorder/updateStatus.do?MATERIALORDER_ID="+ID+"&STATUS="+type;
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>materialorder/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function showPic(value){
			 ...
			 diag.Drag=true;
			 diag.Title ="查看";
			 diag.URL = '<%=basePath%>appuser/getPic?path='+value;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>materialorder/delete.do?MATERIALORDER_ID="+Id+"&tm="+new Date().getTime();
					$.get(url,function(data){
						nextPage(${page.currentPage});
					});
				}
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>materialorder/goEdit.do?pic='+Id;
			 diag.Width = 450;
			 diag.Height = 450;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>materialorder/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>materialorder/excel.do';
		}
	</script>
