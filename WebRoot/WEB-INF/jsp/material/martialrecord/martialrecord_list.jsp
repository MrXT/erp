				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<!-- 检索  -->
							<form action="martialrecord/list.do" method="post" name="Form"
								id="Form">
										...
										<td>
											<div class="nav-search">
												<span class="input-icon"> <input type="text"
													...
													placeholder="这里输入姓名" /> <i
													class="ace-icon fa fa-search nav-search-icon"></i>
												</span>
											</div>
										</td>
										<td style="padding-left: 2px;"><input
											class="span10 date-picker" name="date" id="lastStart"
											value="" type="text" data-date-format="yyyy-mm-dd"
											readonly="readonly" style="width: 88px;" placeholder="日期"
											title="日期" /></td>
										<c:if test="${QX.cha == 1 }">
								...
								<table id="simple-table"
									...
									<thead>
										<tr>
											...
											<th class="center">移交人</th>
											<th class="center">移交部门</th>
											<th class="center">移交时间</th>
											<th class="center">接收人</th>
											<th class="center">接收部门</th>
													...
													<c:forEach items="${varList}" var="var" varStatus="vs">
														<tr>
															<td class='center'><label class="pos-rel"><input
																	...
																	value="${var.MARTIALRECORD_ID}" class="ace" /><span
																	class="lbl"></span></label></td>
															<td class='center' style="width: 30px;">${vs.index+1}</td>
															<td class='center'>${var.MATERIAL_TYPE_NAME}</td>
															<td class='center'>${var.MATERIAL_NAME}</td>
															<td class='center'>${var.BRAND}</td>
															<td class='center'>${(empty var.SERIAL_NUMBER || var.SERIAL_NUMBER=="null")?"-":var.SERIAL_NUMBER}</td>
															<td class='center'>${var.FROM_NAME}</td>
															<td class='center'>${var.FROM_DEPARTMENT_NAME}</td>
															<td class='center'>${fn:substring(var.CTIME,0,16)}</td>
															<td class='center'>${var.TO_NAME}</td>
															<td class='center'>${var.TO_DEPARTMENT_NAME}</td>
														</tr>
...
<%-- 												</c:if></td> --%>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>martialrecord/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>martialrecord/delete.do?MARTIALRECORD_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>martialrecord/goEdit.do?MARTIALRECORD_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>martialrecord/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>martialrecord/excel.do';
		}
	</script>
