				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<!-- 检索  -->
							<form action="phone/list.do?STATUS=${pd.STATUS}" method="post" name="Form" id="Form">
...
<!-- 										</select></td> -->
								...
								<table id="simple-table"
									...
									<thead>
										<tr>
											...
											<th class="center">座机号码</th>
											<th class="center">主线号</th>
											<th class="center">备线号</th>
											<c:if test="${pd.STATUS == 1}">
												<th class="center">姓名</th>
												<th class="center">部门</th>
												<th class="center">门牌号</th>
													...
													<c:forEach items="${varList}" var="var" varStatus="vs">
														<tr>
															<td class='center'><label class="pos-rel"><input
																	type='checkbox' name='ids' value="${var.PHONE_ID}"
																	class="ace" /><span class="lbl"></span></label></td>
															<td class='center' style="width: 30px;">${vs.index+1}</td>
															<td class='center'>${var.CPHONE}</td>
															<td class='center'>${var.MAIN_LINE}</td>
															<td class='center'>${var.BACK_LINE}</td>
															<c:if test="${pd.STATUS == 1}">
																<td class='center'>${var.NAME}</td>
																<td class='center'>${var.DEPARTMENT_NAME}</td>
																<td class='center'>${var.OFFICE_NO}</td>
																<td class='center'>${var.REMARK}</td>
															</c:if>
															<td class='center'>${fn:substring(var.utime,0,16)}</td>
															<td class="center"><c:if
																...
																<div class="btn-group">
																	<c:if test="${QX.edit == 1 && pd.STATUS == 0}">
																		<a class="btn btn-xs btn-success" title="分配号码"
																			onclick="edit('${var.PHONE_ID}',1);">分配号码
																		</a>
																	</c:if>
																	<c:if test="${QX.edit == 1 && pd.STATUS == 1}">
																		<a class="btn btn-xs btn-success" title="更换号码"
																			onclick="edit('${var.PHONE_ID}',2);">更换号码
																		</a>
																		<a class="btn btn-xs btn-success" title="回收号码"
																			onclick="deleteUser('${var.CPHONE}','${var.USER_ID}');">回收号码
																		</a>
																	</c:if>
																	<c:if test="${QX.del == 1 && pd.STATUS == 0}">
																		<a class="btn btn-xs btn-danger"
																			onclick="del('${var.PHONE_ID}');"> <i
																			class="ace-icon fa fa-trash-o bigger-120" title="删除"></i>
																		</a>
																	</c:if>
																	<c:if test="${QX.edit == 1 && pd.STATUS == 0}">
																		<a class="btn btn-xs btn-success" title="编辑"
																			onclick="edit('${var.PHONE_ID}',0);">编辑
																		</a>
																	</c:if>
																</div>
									...
									<table style="width: 100%;">
										<tr>
											<td style="vertical-align: top;"><c:if
												...
												</c:if> <c:if test="${QX.del == 1 && pd.STATUS == 0}">
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>phone/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function deleteUser(value,USER_ID){
			bootbox.confirm("确定要回收座机吗?", function(result) {
				if(result){
					top.jzts();
					$.post("phone/deleteUser",{"CPHONE":value,"USER_ID":USER_ID},function(res){
						nextPage(${page.currentPage});
					});
				}
			});
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>phone/delete.do?PHONE_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id,method){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>phone/goEdit.do?PHONE_ID='+Id+'&method='+method;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>phone/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>phone/excel.do';
		}
	</script>
