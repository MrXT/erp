								...
								<table style="margin-top: 5px;">
									<tr>
										...
										<td style="vertical-align: top; padding-left: 2px;"><select
											...
											id="STATUS" data-placeholder="请选择状态"
											style="vertical-align: top; width: 120px;">
												<option value=""></option>
												<option <c:if test="${pd.STATUS == -2}">selected="selected"</c:if> value="">全部</option>
												<option <c:if test="${pd.STATUS == -1}">selected="selected"</c:if> value="-1">禁用</option>
												<option <c:if test="${pd.STATUS == 0}">selected="selected"</c:if> value="0">未激活</option>
												<option <c:if test="${pd.STATUS == 1}">selected="selected"</c:if> value="1">正常</option>
										</select></td>
										<c:if test="${QX.cha == 1 }">
												...
												<c:if test="${QX.cha == 1 }">
													<c:forEach items="${userList}" var="user" varStatus="vs">
														...
														<tr>
															...
															<td class="center">${user.OFFICE_NO }</td>
															<td class="center">${user.CPHONE }</td>
																...
																<div class="btn-group"><!-- hidden-sm hidden-xs  -->
																	<c:if test="${QX.edit == 1 }">
																		<c:if test="${user.STATUS == 0}">
																			<a class="btn btn-info btn-xs" title="激活"
																				onclick="activeUser('${user.USER_ID}');"> 激活 </a>
																		</c:if>
																		<c:if test="${user.STATUS != 0}">
																			<a class="btn btn-xs"> 已激活 </a>
																		</c:if>
																		...
																		<c:if test="${user.STATUS != -1}">
																			<a class="btn btn-xs btn-info" title="禁用"
																				onclick="forbiddenUser('${user.USER_ID}');"> 禁用
																			</a>
																		</c:if>
																		<c:if test="${user.STATUS == -1}">
																			<a class="btn btn-xs btn-info" title="启用"
																				onclick="qiyongUser('${user.USER_ID}');"> 启用 </a>
																		</c:if>
																	</c:if>
																	<c:if test="${QX.del == 1 }">
																		<%-- 													<a class="btn btn-xs btn-danger" onclick="delUser('${user.USER_ID }','${user.NAME }');"> --%>
...
function activeUser(user_id){
	 ...
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '<%=basePath%>user/goActiveu.do?USER_ID='+user_id;
	 diag.Width = 469;
	 diag.Height = 510;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function forbiddenUser(userId){
	bootbox.confirm("确定要禁用吗?", function(result) {
		if(result) {
			...
			$.ajax({
				type: "POST",
				url: '<%=basePath%>user/forbiddenUser.do',
				data:{"USER_ID":userId},
				dataType:'json',
	...
	function qiyongUser(userId){
		bootbox.confirm("确定要启用吗?", function(result) {
			if(result) {
				top.jzts();
				$.ajax({
					type: "POST",
					url: '<%=basePath%>user/qiyongUser.do',
					data:{"USER_ID":userId},
					dataType:'json',
					//beforeSend: validateData,
					cache: false,
