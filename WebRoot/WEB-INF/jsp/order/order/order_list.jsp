			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="order/list.do?STATUS=${pd.STATUS}" method="post" name="Form" id="Form">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center" width="100px">描述</th>
									<th class="center">申请人</th>
									<th class="center" style="width:50px;">部门</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.MATERIAL_NAMES}</td>
											...
											<td class="center">
												...
												<div class="btn-group">
													...
													<a class="btn btn-xs btn-success" title="派单" onclick="edit('${var.ORDER_ID}',1);">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="派单"></i>
													</a>
													<a class="btn btn-xs btn-danger" title="删除" onclick="del('${var.ORDER_ID}',2);">
														<i class="ace-icon fa fa-trash-o bigger-120" title="删除"></i>
													</a>
													</c:if>
													<c:if test="${QX.edit == 1 && (pd.STATUS == 1 || pd.STATUS==2)}">
													<a class="btn btn-xs btn-success" title="查看" onclick="edit('${var.ORDER_ID}',2);">
														查看
													</a>
													<a class="btn btn-xs btn-success" title="取消工单" onclick="del('${var.ORDER_ID}',1);">
														取消工单
													</a>
													</c:if>
													...
													<a class="btn btn-xs" ><c:if test="${pd.STATUS == 3}">未评价</c:if><c:if test="${pd.STATUS == 4}">已评价</c:if> 
													</a>
													</c:if>
												</div>
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									...
									<a class="btn btn-mini btn-success" onclick="add();">新增工单</a>
									</c:if>
								</td>
	...
	<script type="text/javascript">
		...
		function del(Id,type){
			...
			bootbox.confirm(msg, function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>order/delete.do?ORDER_ID="+Id+"&tm="+new Date().getTime()+"&type="+type;
		...
		function edit(Id,method){
			 ...
			 diag.URL = '<%=basePath%>order/goEdit.do?ORDER_ID='+Id+'&method='+method;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
