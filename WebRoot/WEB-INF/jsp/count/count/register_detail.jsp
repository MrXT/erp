				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<!-- 检索  -->
							<form action="count/registerDetail.do" method="post" name="userForm"
								id="userForm">
								<table style="margin-top: 5px;">
									<tr>
										<td style="vertical-align: top; padding-left: 2px;"><select
											class="chosen-select form-control" name="type"
											id="type" data-placeholder="请选择状态"
											style="vertical-align: top; width: 120px;">
												<option value=""></option>
												<option <c:if test="${pd.type == 1}">selected="selected"</c:if> value="1">当月</option>
												<option <c:if test="${pd.type == 2}">selected="selected"</c:if> value="2">本季度</option>
												<option <c:if test="${pd.type == 3}">selected="selected"</c:if> value="3">半年</option>
										</select></td>
										<td style="vertical-align: top; padding-left: 2px;"><select
											class="chosen-select form-control" name="status"
											id="status" data-placeholder="请选择状态"
											style="vertical-align: top; width: 120px;">
												<option <c:if test="${pd.status == 2}">selected="selected"</c:if> value="2">不活跃</option>
												<option <c:if test="${pd.status == 1}">selected="selected"</c:if> value="1">活跃</option>
										</select></td>
											...
											<c:when test="${not empty userList}">
													<c:forEach items="${userList}" var="user" varStatus="vs">
														...
														<tr>
															...
															<td class="center">${user.officeNo}</td>
