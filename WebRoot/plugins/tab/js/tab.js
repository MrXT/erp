...
function TabView(f) {
    ...
    if ($("#scrollContent").attr("childScrollContent") == "true") {
        ...
        i = '<iframe id="{0}" scrolling="no"  style="overflow-x:hidden;" allowTransparency="true" frameborder="0" width="100%" height="100%" src="{1}"></iframe>'
    }
    ...
    function d(l, k) {
        ...
        a.current_page.show();
        $("#test").html($("#" + l).find(".tab_title").attr("title"));
    ...
    this.add = function (n) {
        ...
        if ($("#" + m.id).length > 0) {
        	...
        	var alltitle = m.title;
        	m.title = m.title.split("-").length==1?m.title:m.title.split("-")[1];
            ...
            $("#" + m.id).find(".tab_title").attr('title',alltitle);
            this.activate(m.id)
        }
    };
    this.update = function (k) {
        var l = k.id;
        k.title = k.title.split("-").length==1?k.title:k.title.split("-")[1];
